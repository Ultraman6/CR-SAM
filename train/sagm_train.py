...
parser.add_argument('--exp_name', default='sagm', action='store_true')
...
def run_one_epoch(phase, loader, model, criterion, optimizer, args):
    ...
    for batch_idx, inp_data in enumerate(loader, 1):
        ...
        # # 计算范数
        # h_norm = torch.norm(grad_h, p=2).item()
...
def main(args):
    ...
    optimizer = GSAM(params=model.parameters(), base_optimizer=base_optimizer, model=model, alpha=args.alpha,
